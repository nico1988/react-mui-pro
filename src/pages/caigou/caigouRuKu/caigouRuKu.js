...
import AddRuKuDialog from './components/ruku-add';
...
export default function CaigouRuKu() {
  ...
  const getList = (p) => {
    getCaiGouRuKuList(p || { ...pagination, ...search }).then((r) => {
      setList(r.data);
      setTotal(r.total);
    });
  };
  ...
  const onRevertAudit = () => {
    revertAuditRuKu(selected).then((r) => {
      dispatch(
        setGlobalMessage({
          variant: 'success',
          msg: '操作成功！',
  ...
  return (
    <Page title="采购管理: 采购入库">
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs heading="采购入库" links={[{ name: '采购管理' }, { name: '采购入库' }]} />
        <Card>
                  ...
                  {list.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="left">{row.inventoryNo}</TableCell>
