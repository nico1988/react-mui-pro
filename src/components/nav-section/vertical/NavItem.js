...
export function NavItemRoot({ item, isCollapse, open = false, active, onOpen }) {
  ...
  const renderContent = (
    <>
      ...
      <ListItemTextStyle disableTypography primary={getI18nText(title, 'menu') || title} isCollapse={isCollapse} />
      {!isCollapse && (
        <>
          {info && info}
...
export function NavItemSub({ item, open = false, active = false, onOpen }) {
  ...
  const renderContent = (
    <>
      ...
      <ListItemText disableTypography primary={getI18nText(title, 'menu') || title} />
      {info && info}
      {children && <ArrowIcon open={open} />}
    </>
  );
